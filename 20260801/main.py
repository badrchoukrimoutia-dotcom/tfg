...
from std_msgs.msg import Header, String # importamos la cabecera estandar y el mensaje de texto de /active_constraints
...
import json # importamos json para leer las restricciones/preferencias activas que llegan por topic
...
from cost_module import load_cost_library, complex_cost_injection, apply_semantic_overlay # importamos las funciones de costes, incluida la superposicion semantica
...
class CostInjectionNode(Node): # definimos el nodo encargado de generar y publicar el mapa de costes
    def __init__(self): # constructor donde preparamos todo el nodo
        ...
        # el mapa de costes es ESTATICO (se calcula una sola vez aqui abajo y no vuelve a cambiar), asi que
        # basta con publicarlo una vez: gracias al QoS TRANSIENT_LOCAL definido arriba, cualquier suscriptor
        # que llegue mas tarde (nav2, rviz2) recibe automaticamente esa ultima copia guardada. Republicarlo
        # sin parar cada 2 segundos para siempre solo gastaba cpu y ancho de banda sin ningun beneficio.
        timer_period = 2.0 # reintentamos cada 2 segundos SOLO hasta conseguir publicar la primera vez
        self.timer = self.create_timer(timer_period, self.publish_map) # creamos un temporizador que llama a la funcion de publicacion

        # suscriptor a las restricciones/preferencias activas: cuando cambian, recalculamos y republicamos el mapa de costes
        self.constraints_subscriber = self.create_subscription(String, '/active_constraints', self.constraints_callback, 10)

        self.get_logger().info('Starting cost injection') # avisamos por terminal que el nodo arranca
        self.resolution = 0.1 # cada celda de la matriz equivale a 10 cm en el mundo real (usado al publicar y al recalcular)
        self.cost_library = load_cost_library('costes_preestablecidos.json') # cargamos la biblioteca de costes desde el json

        self.detected_objects = load_environment_json('ejemplo_natalia.json') # cargamos el json del entorno con habitaciones y objetos

        # el costmap BASE (paredes + muebles) es estatico y no vuelve a cambiar; lo guardamos aparte para poder
        # recalcular sobre el, sin perderlo, cada vez que llegan restricciones/preferencias nuevas por /active_constraints
        self.base_costmap = None # inicializamos la matriz de costes vacia hasta que se genere
        if self.detected_objects and self.cost_library: # comprobamos que se han cargado correctamente los datos
            ...
            self.base_costmap, self.map_w, self.map_h, self.origin_x, self.origin_y = complex_cost_injection(self.detected_objects, self.cost_library) # generamos el mapa de costes
    ...
    def publish_map(self): # funcion que se ejecuta cada 2 segundos SOLO hasta conseguir la primera publicacion (arranque)
        if self.base_costmap is None: # si todavia no hay matriz generada
            return # no hacemos nada y salimos

        self.publish_costmap(self.base_costmap) # al arrancar todavia no hay restricciones/preferencias activas
        # el mapa base no cambia por si solo (solo cuando llega algo por /active_constraints), asi que con esta
        # unica publicacion inicial (mas el QoS transient_local) es suficiente; las siguientes las dispara constraints_callback
        self.timer.cancel()

    def constraints_callback(self, msg): # se ejecuta cada vez que cambian las restricciones/preferencias activas
        if self.base_costmap is None: # si el mapa base no se pudo generar, no hay nada sobre lo que recalcular
            return

        try: # el json llega desde otro nodo (navigation_node.py); lo tratamos como datos externos, puede fallar
            data = json.loads(msg.data) # convertimos el texto recibido en un diccionario
        except Exception as e: # si el mensaje no es un json valido
            self.get_logger().error(f"Invalid /active_constraints payload: {e}") # informamos del error por terminal
            return # no tocamos el mapa publicado

        constraints = data.get('restricciones', []) # lista de restricciones activas (cada una con su "target")
        preferences = data.get('preferencias', []) # lista de preferencias activas (cada una con su "target")

        # recalculamos siempre partiendo del costmap base (nunca del ultimo publicado), para que retirar una
        # restriccion la haga desaparecer de verdad en vez de acumularse sobre el coste ya pintado antes
        updated_costmap = apply_semantic_overlay(
            self.base_costmap, self.detected_objects, self.origin_x, self.origin_y, self.resolution, constraints, preferences
        )
        self.get_logger().info(
            f"Recalculando mapa de costes: {len(constraints)} restriccion(es), {len(preferences)} preferencia(s) activas."
        ) # avisamos del recalculo
        self.publish_costmap(updated_costmap) # publicamos la version actualizada

    def publish_costmap(self, costmap): # normaliza una matriz de costes a la escala de nav2 y la publica en /mapa_de_costes
        ...
        msg.info.resolution = self.resolution # indicamos que cada celda mide 10 cm
        ...
        data = costmap.astype(np.int16) # convertimos la matriz a int16 para tener margen durante los calculos
        ...
        self.get_logger().info('Publishing cost map') # avisamos por terminal de la publicacion
